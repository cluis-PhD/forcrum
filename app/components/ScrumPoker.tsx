import { ArrowLeft, Users, Eye, EyeOff, RotateCcw, Save, ChevronDown, Trophy } from 'lucide-react';
...
export function ScrumPoker({ onBack }: ScrumPokerProps) {
  ...
  const syncVotesFromBackend = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/votes?storyId=${currentStoryId}`,
        {
          method: 'GET',
          headers: {
            ,
  ...
  const saveVoteToBackend = async (voteValue: string) => {
    ...
    try {
      ...
      // ✅ CORREÇÃO: Buscar TODAS as stories (porque o storage é em RAM e algumas stories antigas podem não existir)
      const allStoriesUrl = `/api/stories`;
      console.log('[ScrumPoker] 🌐 Buscando todas as stories:', allStoriesUrl);
      
      const storiesResponse = await fetch(allStoriesUrl, {
        headers: { // Authorization removed (using Neon API) }
      });
      
      if (!storiesResponse.ok) {
        console.error('[ScrumPoker] ❌ Erro ao buscar stories - Status:', storiesResponse.status);
        throw new Error(`Erro ao buscar stories: ${storiesResponse.status}`);
      }
      
      const storiesData = await storiesResponse.json();
      console.log('[ScrumPoker] 📦 Stories recebidas:', storiesData);
      
      if (!storiesData.success || !storiesData.stories) {
        throw new Error('Erro ao buscar stories');
      }
      
      // Encontrar a story atual
      const story = storiesData.stories.find((s: any) => s.id === currentStoryId);
      
      if (!story) {
        console.error('[ScrumPoker] ❌ Story não encontrada na lista:', currentStoryId);
        console.error('[ScrumPoker] 📋 Stories disponíveis:', storiesData.stories.map((s: any) => s.id));
        throw new Error('Story não encontrada');
      }
      
      console.log('[ScrumPoker] ✅ Story encontrada:', { id: story.id, title: story.title });
      
      // ✅ 2. Atualizar o array votes com o novo voto
      ...
      // ✅ CORREÇÃO: Converter voto para número (ignorar valores especiais)
      let voteNumber = 0;
      if (voteValue !== '?' && voteValue !== '☕') {
        // Se for T-Shirt, converter para número
        if (votingSystem === 'tshirt') {
          const sizeMap: Record<string, number> = { 
            'XXS': 1, 'XS': 2, 'S': 3, 'M': 5, 'L': 8, 'XL': 13, 'XXL': 21 
          };
          voteNumber = sizeMap[voteValue] || 0;
        } else {
          voteNumber = parseInt(voteValue) || 0;
        }
      }
      ...
      console.log('[ScrumPoker] 🗳️ Votos atualizados:', filteredVotes);
      
      // ✅ 3. Salvar no backend
      const updateUrl = `/api/stories/${currentStoryId}`;
      console.log('[ScrumPoker] 💾 Salvando voto na URL:', updateUrl);
      
      const updateResponse = await fetch(updateUrl, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          // Authorization removed (using Neon API)
        },
        body: JSON.stringify({
          votes: filteredVotes
        })
      ...
      if (!updateResponse.ok) {
        console.error('[ScrumPoker] ❌ Erro ao atualizar story - Status:', updateResponse.status);
        const errorText = await updateResponse.text();
        console.error('[ScrumPoker] ❌ Erro detalhado:', errorText);
        throw new Error(`Erro ao salvar voto no backend: ${updateResponse.status}`);
      }
      
      const updateData = await updateResponse.json();
      console.log('[ScrumPoker] ✅ Resposta do update:', updateData);
      
      console.log('[ScrumPoker] ✅ Voto salvo no backend com sucesso!', filteredVotes);
