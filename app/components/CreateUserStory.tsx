...
export function CreateUserStory({ onBack, onSave, sprintId: initialSprintId, sprintName: initialSprintName, courseId: initialCourseId, courseName: initialCourseName }: CreateUserStoryProps) {
  ...
  const loadCourses = async () => {
    try {
      ...
      if (error instanceof TypeError && error.message === 'Failed to fetch') {
        console.error('Verifique se as credenciais do Neon estão corretas');
        console.error('URL tentada:', `/api/courses`);
      }
      setCourses([]);
    }
  };
  ...
  const loadSprints = async () => {
    try {
      ...
      if (error instanceof TypeError && error.message === 'Failed to fetch') {
        console.error('❌ Verifique se as credenciais do Neon estão corretas');
        console.error('❌ URL tentada:', `/api/sprints`);
        toast.error('Erro de conexão. Verifique as credenciais do Neon.', { duration: 5000 });
      } else {
  ...
  const loadTeams = async () => {
    try {
      ...
      if (error instanceof TypeError && error.message === 'Failed to fetch') {
        console.error('❌ Verifique se as credenciais do Neon estão corretas');
        console.error('❌ URL tentada:', `/api/teams`);
        toast.error('Erro de conexão. Verifique as credenciais do Neon.', { duration: 5000 });
      } else {
